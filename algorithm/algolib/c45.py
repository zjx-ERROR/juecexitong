...
def train(data):
    ...
    conn = pymysql.connect(host=mhost, port=mport, user=muser, passwd=mpasswd, db=mdb)
    cursor = conn.cursor()
    for de in data['evaluate']:
        cursor.execute('select name,func_name,definition from algorithmic_assessment where id = "%d"' % de)
        cf = cursor.fetchone()
        evalist.append(
            {'name': cf[0], 'value': importlib.import_module(cf[1]).evaluate(testset_tmp, test_reslult), 'ins': '','conclusion':cf[2]})
